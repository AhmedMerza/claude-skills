---
name: spinoff
description: After a branch is finished, harvest the work it made CHEAP — the helper you wrote that four other sites now hand-roll, the plumbing that turned a deferred feature small, the pattern that just split the codebase in two. Produces a follow-up backlog (0–3 candidates, feeding /issue), NEVER commits on the current branch. Invoke with /spinoff after /ship-check and before /mr-create, or when the user asks "what else should we do while we're here / what did this unlock / anything to follow up on". This is ship-check's optional twin — that finds what's MISSING and blocks the merge; this finds what's now CHEAP and blocks nothing. Restraint-gated: "nothing spun off" is the common and correct answer, and a finding that would have been equally valid before this branch is not a finding.
---
